...
function buildV11PromptUnsealed(options) {
  ...
  const prompt = [
    ...
    ...(isChangedFactPhase(phase) ? [
      'For this review, changedFactDetected means the observed fact materially requires reconsidering the earlier decision: return true and copy observedFact.id to changedFactId when it does; return false and copy observedFact.id to changedFactId when it does not (an observed fact that has no bearing on the earlier decision does not require reconsideration, so return false); return null with changedFactId null only when the supplied evidence is insufficient to decide.'
    ] : []),
    ...(phase === 'E' ? [
