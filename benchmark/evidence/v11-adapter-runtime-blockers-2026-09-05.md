...
| D4 | ~~Is a retry floor the library does not let us disable acceptable, or does `automatic_retries: 0` stand?~~ **Answered 2026-09-05:** it stands. The premise was false - Mem0's retries are disableable, and one request per call was observed on the wire. What was missing was detection: `reconcileProviderEvidence` had no caller outside its own test. It now judges a real ledger, with a control that injects a retry and requires it to be named. See `v11-retry-reconciliation-2026-09-05.md` | resolved |
