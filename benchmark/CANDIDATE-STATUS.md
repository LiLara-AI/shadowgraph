...
| F31 | Nothing in the frozen prompt contract says what `decisionId` should contain. The schema does request the field (`"decisionId":"string|null"`) and the record id **is** in native context as `record.id` - an earlier statement that the model "is never given one nor asked to mint one" was wrong in both halves. What is missing is the semantics: the system prompt never mentions the field, the schema gives a type only, so a model returning `null` complied with everything it was told. Observed: `null` x152, `'D001'` x28 (every one at `nativeContextCount == 0`, so invented), a real `decision:<hex>` x4 | **Open - dependency identified: an owner-authorised amendment.** Both levers are frozen: the outer system prompt is hashed into `outerPromptBinding.systemSha256`, and the response schema is verbatim in `preregistration.json`. Options and a recommendation are in `benchmark/evidence/v11-f31-amendment-proposal-2026-09-06.md`, which is a proposal and is adopted by nothing |
| F32 | The harness penalised exactly the arms that retrieve. `standardizedDecisionRecord` stores each decision response verbatim as the record's content, adapters return record content verbatim as native context, and `canonicalJson` key order put `"changedFactDetected":null` first - so the model read a null answer to the question it was being asked. 64 of 64 nulls occurred in arms with retrieved context; 0 of 12 in the control. The confound's direction differs by metric, and an earlier flat claim that it favoured the control was wrong | **Cleared.** `serializeNativeContext` drops `changedFactDetected`, `changedFactId` and the prior `decisionId` echo before rendering, matching the key at any depth, leaving `record.id`, the stored record, the adapter protocol and both prompt-binding hashes untouched - both hashes recomputed after the fix still match run 002's recorded values. 14 tests, 13 mutants killed. **A record serialised into a string is not covered: see F37** |
...
| F37 | Key-based redaction cannot reach inside a string. Cognee's retrieve returns `{search_result, dataset_id, dataset_name}` with the record encoded as JSON inside `search_result`, so a provisioned cognee arm would still see the pre-filled answer fields while the other arms are redacted - F32 back for one of the seven required arms. No exposure in run 002: cognee returned zero native context, 40 of its 44 units having failed | **Open.** Remedy identified: stop writing the three probe-answer fields to a decision record at all, which needs a 12-field record-content contract distinct from the 15-field response schema in `standardizedDecisionRecord` and both exact-keys validators. Attempted and reverted this cycle - correct, but it moves a contract shared with all seven adapters and pinned by hard-coded digests in two languages |
| F38 | `npm run benchmark:test` is red and cannot be used as a gate. `test/benchmark-v11-definition.test.js:696` asserts "this candidate has produced no benchmark result", which was true when it was written and is false now that run 002 exists in the gitignored `benchmark/results/`. Because the script is `node --test ... && npm run benchmark:test:python`, the failing JS half means **the 139 Python tests never run under the registered command** - verified: zero `Ran N tests` lines in its output | **Open.** The guard is correct and is not to be weakened; its premise was superseded by the candidate actually running. Until it is reconciled, the two halves must be run separately, which is how every count in the findings report and handoff was produced |
