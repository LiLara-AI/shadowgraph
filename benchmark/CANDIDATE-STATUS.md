...
| F30 | The scorer awarded passing scores to absent answers, three ways. `falseAlertRate` divided by the probes it found and counted a null as a negative, against `preregistration.json:280` ("Null, malformed, or missing is a failed unit, not a negative prediction"), so an arm answering none of the three probes posted the best rate in the run. `projectIsolation`/`userIsolation` returned **1 - the best score on a hard security gate** - for a probe that returned nothing, against a rule reading "1 only when ... **and** persisted-state inspection confirms". And that persisted-state half was never consulted at all: it read `leakedRecordIds` and `persistedLeak`, neither of which any adapter in this repository writes | Cleared: the rate is N/A rather than 0 when any probe failed, with probe counts beside it; a 1 now requires both halves; the real evidence (`verified` plus two match counts) is read, and a match count overrules an adapter's own `verified`. Frozen v1.0 scores unchanged. 28 tests, 17 mutants killed |
| F31 | `decisionRetrievalAccuracy` is **structurally unmeasurable as designed**: no decision id exists as a fixture, and the model is never given one nor asked to mint one. Observed across run 002: `null` x152, `'D001'` x28 (every one with `nativeContextCount == 0` - a hallucinated placeholder), a real `decision:<hex>` x4 (copied out of retrieved context). The metric is 0 for every arm, including arms that recall everything else correctly | **Open - requires an amendment before any scored run.** Must not be "fixed" in the scorer: changing the rule so the current data passes is exactly what the methodology forbids |
| F32 | The harness penalises exactly the arms that retrieve. `standardizedDecisionRecord` stores each decision response verbatim as the memory record's content, adapters return record content verbatim as native context, so a memory arm's D-phase prompt carries filled copies of the object being requested - each beginning `"changedFactDetected":null` under `canonicalJson` key order - and the 7B model transcribes it. 48 of 48 `D_FALSE` nulls occur in arms with retrieved context; 0 of 12 in the control. Memory arms match their own prior answer on 14 of 15 fields, the control on 2 of 15. Flipping only that field in the retrieved records, at identical `prompt_tokens`, flips the answer | **Open - blocks any comparative D-phase claim.** Failure layer is the harness prompt/record format plus 7B capacity; not a ShadowGraph failure and not a competitor failure |
...
| F34 | All three amendments assert `repetitionsChanged: false`, while `preregistration-amendment-002.json` sets `commonExecution.repetitions: 2` against the preregistration's 3. Both files use the identical key path with different values and nothing states that these are two protocols - the scored benchmark and the non-scored candidate acceptance | **Open**, documentation. The 2-repetition design is correctly authorised by amendment-002's `candidateAcceptance` block; what is missing is any sentence saying the two counts belong to different plans |
