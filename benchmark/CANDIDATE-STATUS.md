...
| Gate | Command | Result |
| --- | --- | --- |
| Full repository | `npm test` | **2042 / 2042 pass**, 0 fail, 20 suites |
| Benchmark focused | `npm run benchmark:test` | **887 / 887 pass**, 0 fail |
| v1.1 suites only | `node --test test/benchmark-v11-*.test.js` | **744 / 744 pass**, 0 fail |
...
glob. The table is generated by `scripts/mutation-table.sh`, which drives
`scripts/mutate.cjs` to remove one guard at a time and count failures. It aborts
if a mutation's anchor stops matching, if fewer than seven measurements land, or
if the tree is dirty before or after — a mutation that silently stopped applying
would otherwise report zero failures and read as a guard nobody needs.
