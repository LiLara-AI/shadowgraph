...
test('compact MCP advertises the workflow surface including remember and recall', async (t) => {
  ...
  const result = await response; child.kill(); assert.equal(result.result.tools.length, 14); assert.ok(result.result.tools.some((tool) => tool.name === 'shadowgraph_context')); assert.ok(result.result.tools.some((tool) => tool.name === 'shadowgraph_remember')); assert.ok(result.result.tools.some((tool) => tool.name === 'shadowgraph_recall'));
});
