...
test('bounded artifacts propagate proven 1-7 character aliases across ancestors, siblings, text, and every string array', async (t) => {
  const directory = await scratchDirectory(t, 'shadowgraph-adapter-alias-boundary-');
...
test('credential URL names decode through three rounds and a fourth round fails closed', async (t) => {
  const directory = await scratchDirectory(t, 'shadowgraph-adapter-url-name-rounds-');
...
test('encoded credential names sanitize short and long sibling echoes across every retained execution artifact', async (t) => {
  const directory = await scratchDirectory(t, 'shadowgraph-encoded-name-artifacts-');
...
test('ambiguous encoded credential names fail closed across success, nonzero, malformed, timeout, raw, and aggregate evidence', async (t) => {
  const directory = await scratchDirectory(t, 'shadowgraph-ambiguous-name-artifacts-');
...
test('known configured and request secrets of 1-7 characters stay invocation-scoped across every retained artifact', async (t) => {
  const directory = await scratchDirectory(t, 'shadowgraph-configured-short-secrets-');
...
test('512 and 513+ credential names close success, nonzero, malformed, timeout, raw, and aggregate artifacts', async (t) => {
  const directory = await scratchDirectory(t, 'shadowgraph-adapter-name-lengths-');
