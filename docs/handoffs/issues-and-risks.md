...
| **P1-14** | critical | SQLite restore closed the live handle and renamed the candidate over the configured path before `DatabaseSync` reopen and schema preparation. Post-replacement open/prepare failure could lose old state; recovery inspection could create an empty missing destination; corrupt journal folds could be accepted; and overlapping HTTP context/write calls could mutate only the in-memory graph. | fixed with verified `VACUUM INTO` source/live snapshots, read-only existing-destination inspection, preserved recovery copy, reopen/prepare/load/payload confirmation, journal/live projection checks, mandatory shared validation on every surface, HTTP restore/mutation serialization, exact retained-artifact reporting, unconfirmed-recovery errors, and Node 22 fault-injection/interface coverage; guarantees are process-level only |
