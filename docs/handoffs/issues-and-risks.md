...
| --- | --- | --- | --- |
| **P1-11** | high | JSON restore could write a syntactically valid but semantically malformed payload over the live file; direct JavaScript could omit the optional validator entirely. | fixed with mandatory shared domain/journal validation before atomic rename on direct and transport paths; regression test proves old-file preservation |
...
| **P1-14** | critical | SQLite restore closed the live handle and renamed the candidate over the configured path before `DatabaseSync` reopen and schema preparation. Post-replacement open/prepare failure could lose old state; recovery inspection could create an empty missing destination; corrupt journal folds could be accepted; overlapping HTTP context/write calls could mutate only the in-memory graph; and the HTTP process remained live after unconfirmed recovery even though graph and disk could diverge. | fixed with verified `VACUUM INTO` source/live snapshots, read-only existing-destination inspection, preserved recovery copy, reopen/prepare/load/payload confirmation, journal/live projection checks, mandatory shared validation on every surface, HTTP restore/mutation serialization, exact retained-artifact reporting, unconfirmed-recovery errors plus a server-wide `503` degraded latch, and Node 22 fault-injection/interface coverage; guarantees are process-level only |
...
| **P2-19** | medium | Project-scoped redaction returned other projects' idempotency entries and review signals, and did not redact `idempotencyKey`/cache-key values. | fixed with project filtering for idempotency and review signals plus key-value redaction; adversarial test added |
