...
| **P1-14** | critical | SQLite restore closed the live handle and renamed the candidate over the configured path before `DatabaseSync` reopen and schema preparation. A post-replacement open/prepare failure could return an error after the old database had already been lost. Direct/CLI/MCP restore also lacked mandatory domain validation, and an overlapping HTTP write could mutate only the in-memory graph. | fixed with verified `VACUUM INTO` source/live snapshots, preserved recovery copy, reopen/prepare/load/payload confirmation, mandatory shared validation on every surface, HTTP restore/write serialization, explicit retained-artifact reporting, unconfirmed-recovery errors, and Node 22 fault-injection/interface coverage; guarantees are process-level only |
