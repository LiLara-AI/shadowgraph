...
- Reject corrupt journal folds and any records/facts/relations/idempotency projection that disagrees with live restored state. Every missing sequence must be enumerated by a later persisted `project.purged` entry with `mode: 'hard'`; each `removedJournalSequences` value is a positive safe integer, unique in that marker, strictly earlier than the marker, and an actual replay-range gap. Present/unrelated, future/early, duplicate, and incomplete ledgers fail closed with bounded range arithmetic. Documented schema-1–4 raw-ID purge markers are accepted only after a projection-equivalent payload-tombstone migration; when that migration changes a baseline collection, its payload/envelope are canonicalized and identifying or unknown metadata is discarded. Unrelated markers and untouched baselines are not globally sanitized.
- Normalize a legacy SQLite snapshot only in an isolated staged database. Use SQLite-native `secure_delete=ON` before the transactional relational replacement, commit or roll back that replacement atomically, checkpoint, switch to a standalone rollback journal, and run `VACUUM` only **after** the transaction to remove freelist and unused legacy pages; checkpoint again before rename. Never scrub database bytes manually. A checkpointed source is opened immutable so restore does not create source WAL/SHM files; an existing source WAL is read normally so committed state is not omitted. The installed normalized database and its backup must contain none of the erased bytes in live cells, free pages, unused page regions, WAL, SHM, or rollback journals.
- On a caught post-replacement failure, restore and reopen the old payload. Report `json_restore_recovery_unconfirmed` / `sqlite_restore_recovery_unconfirmed` rather than letting later artifact-inspection errors replace the primary recovery outcome, and latch HTTP/MCP degraded before a later graph read or write can observe or mutate potentially divergent state. Authenticated health/error diagnostics may list confirmed `retainedArtifacts` and separate `{ path, code }` `unknownArtifacts`; the public static dashboard must not expose them.
- After successful JSON replacement or confirmed rollback, attempt and re-inspect cleanup for every temporary, recovery, and rollback path. Cleanup anomalies report `artifactCleanup.status` plus path/code errors; only a path confirmed to exist belongs in `retainedArtifacts`, and an inspection failure belongs in `unknownArtifacts`. A delete-that-then-throws must not be falsely reported as retained.
